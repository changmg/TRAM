...
from quant.quant_layer import StraightThroughLayer, QuantLayer
...
def is_absorbing(m):
    ...
    return isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear) or isinstance(m, QuantLayer)
...
def search_fold_and_remove_bn(model):
    ...
    for n, m in model.named_children():
        if is_bn(m) and is_absorbing(prev):
            ...
            setattr(model, n, StraightThroughLayer())
        elif is_absorbing(m):
            prev = m
        else:
