...
class QuantLayer(nn.Module):
    ...
    def __init__(self, org_module: Union[nn.Conv2d, nn.Linear], weight_quant_params: dict = {}, act_quant_params: dict = {}):
        ...
        if isinstance(org_module, nn.Conv2d):
            ...
            # self.app_fwd_func = self_ops.approx_conv2d_baseline_op_for_layer
    ...
    def forward(self, input: torch.Tensor):
        ...
        if not self.use_appmult or self.fwd_func is F.linear: # do not use AppMult in linear layer
            out = self.fwd_func(input_fq, weight_fq, bias=self.bias, **self.fwd_kwargs)
        else: # approximate convolution only
            ...
            out = scale_a * scale_w.view(self.view_shape) * (
                ...
                # self.app_fwd_func(input_int, weight_int, bias=None, **self.fwd_kwargs, layer_id=self.layer_id)
    ...
    def add_trainable_indicators(self):
        ...
        self.gamma_free = torch.nn.Parameter(torch.ones((self.num_max_discard_cols,), dtype=torch.float32, device='cuda') * ind_init_value)
